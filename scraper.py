...
def extraer_resultados_parley_forzado():
    mapa_resultados = {}
    
    # Encabezados estrictos para evitar respuestas de caché o bloqueos silenciosos
    headers = {
    ...
    for loteria_nombre, url_base in LOTERIAS_PARLEY.items():
        try:
            # Parámetro dinámico para forzar al servidor a soltar datos frescos
            url = f"{url_base}?refresh={datetime.now().timestamp()}"
            ...
            print(f"Error forzando extracción de {loteria_nombre}: {e}")
...
def generar_base_datos():
    ...
    datos_reales = extraer_resultados_parley_forzado()
    resultados = []
...
if __name__ == "__main__":
    ...
    print("Extracción forzada completada con éxito.")
